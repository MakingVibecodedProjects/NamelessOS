...
- `src/process/process.h` — `process_t` (pid, state, name[32], kstack, ctx, next), `process_init()`, `kthread_create()`, `process_exit()`, `process_current()`, `process_set_current()`, `process_get()`
...
- **No context switch yet** — that is the scheduler's job; process.c only manages the table and stacks
